...
<!-- Fetched verbatim via WebFetch tool. This is the source of truth for Chapter 3. -->

# Chapter 3: How to Run a Program

[Edit on GitHub](https://github.com/hackclub/putting-the-you-in-cpu/tree/main/src/content/chapters/3-how-to-run-a-program.mdx)
...
This often involves looking for [magic numbers](https://en.wikipedia.org/wiki/Magic_number_\(programming\)#Format_indicator) in the buffer, attempting to decode the start of the program (also from the buffer), and/or checking the file extension. If the handler does support the format, it prepares the program for execution and returns a success code. Otherwise, it quits early and returns an error code. The kernel tries the `load_binary()` function of each binfmt until it reaches one that succeeds. Sometimes these will run recursively; for example, if a script has an interpreter specified and that interpreter is, itself, a script, the hierarchy might be `binfmt_script` > `binfmt_script` > `binfmt_elf` (where ELF is the executable format at the end of the chain).
...
**DIAGRAM (linux-shebang-truncation.png):** A diagram depicting shebang truncation. A large array of bytes from a file named file.bin. The first 256 bytes are highlighted and labeled "Loaded into buf," while the remaining bytes are translucent and labeled "Ignored, past 256 bytes."
...
[Continue to Chapter 4: Becoming an Elf-Lord »](becoming-an-elf-lord)
