<!-- SOURCE: https://cpu.land/lets-talk-about-forks-and-cows -->
<!-- Fetched verbatim via WebFetch tool. This is the source of truth for Chapter 6. -->

# Chapter 6: Let's Talk About Forks and Cows

[Edit on GitHub](https://github.com/hackclub/putting-the-you-in-cpu/tree/main/src/content/chapters/6-lets-talk-about-forks-and-cows.mdx)
...
Process forking can be a bit hard to wrap your head around. From this point on I will assume you've figured it out.
...
**KEY VISUAL CONCEPT FOR ANIMATION — COW (copy-on-write) pages:** Fork duplicates page tables (not RAM) so parent + child point to the *same* physical memory, marked read-only. On write, a hardware interrupt (segfault) fires, the kernel copies the page, marks it writable, and retries the write.
...
**DIAGRAM (init-process-tree.png) — KEY VISUAL CONCEPT FOR ANIMATION:** A tree of processes. The root node is labeled "init." All child nodes are unlabeled but implied to be spawned by the init process.
...
**KEY VISUAL CONCEPT FOR ANIMATION — Boot sequence:** 4-step linear sequence: motherboard firmware -> bootloader -> kernel init -> init process (userland). Good candidate for a horizontal scroll-driven timeline animation.

### Initializing Linux

On Linux, the bulk of step 3 (kernel initialization) occurs in the `start_kernel` function in `init/main.c`. This function is over 200 lines of calls to various other init functions. At the end of `start_kernel` a function named `arch_call_rest_init` is called:
...
On Linux, the init program is almost always located at or symbolic-linked to `/sbin/init`. Common inits include [systemd](https://systemd.io/), [OpenRC](https://wiki.gentoo.org/wiki/OpenRC/openrc-init), and [runit](http://smarden.org/runit/). `kernel_init` will default to `/bin/sh` if it can't find anything else — and if it can't find `/bin/sh`, something is TERRIBLY wrong.

*MacOS has an init program, too! It's called launchd and is located at `/sbin/launchd`.*
...
`kernel/fork.c` contains most of the code for forking processes. The start of that file helpfully points to the right place to look:
...
It looks like this `copy_page_range` function takes some information about a memory mapping and copies the page tables. This is also where pages are set to be read-only to make them COW pages. It checks whether it should do this by calling a function called `is_cow_mapping`.

`is_cow_mapping` is defined back in `include/linux/mm.h`, and returns true if the memory mapping has flags that indicate the memory is writeable and isn't shared between processes. Shared memory doesn't need to be COWed because it is designed to be shared.
...
[Continue to Chapter 7: Epilogue »](epilogue)
